...
int main(int argc, char* argv[]) {
    ...
    Logger logger("char-log.log", DEBUG);
    char logBuf[128];
    ...
    TestLimits limits = getLimits<avx::Char256::storedType>();
    ...
    for(int i = 0; i < std::thread::hardware_concurrency() - 1; ++i){
        ...
        if(procid < 0){
            ...
            divisionWorker<avx::Char256, char>(msgid, slices[i]);
            return 0;
        }
        else {
    ...
    int status;
    pid_t ret_pid;
    std::ofstream csvFile("mismatch_char.csv", std::ios_base::trunc);
