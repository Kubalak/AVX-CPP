...
int main(int argc, char* argv[]) {
    ...
    Logger logger("ushort-log.log", DEBUG);
    char logBuf[128];
    ...
    for(int i = 0; i < std::thread::hardware_concurrency() - 1; ++i){
        ...
        if(procid < 0){
            ...
            divisionWorker<avx::UShort256, unsigned short>(msgid, slices[i]);
            return 0;
        }
        else {
    ...
    int status;
    pid_t ret_pid;
    std::ofstream csvFile("mismatch_ushort.csv", std::ios_base::trunc);
